...
def renderStudentsList(request):
    ...
    connection=mysql.connector.connect(host='localhost',user='root',password='',database='race')
    cursor=connection.cursor()
    print('SELECT name from students s INNER JOIN '+table+' r on s.id = r.studentId where r.'+val+'="'+id+'"')
...
def updateBlood(request):
    ...
    connection=mysql.connector.connect(host='localhost',user='root',password='',database='race')
    cursor=connection.cursor()
    cursor.execute('update requirement set recipient="'+name+'", contactnumber="'+contact+'", requirement="'+blood+'" where id="'+id+'"' )
    connection.commit()
...
def updateJob(request):
    ...
    connection=mysql.connector.connect(host='localhost',user='root',password='',database='race')
    cursor=connection.cursor()
    print('UPDATE jobs set position="'+position+'",description="'+description+'",lastdate="'+lastdate+'" WHERE id="'+id+'"')
    cursor.execute('UPDATE jobs set position="'+position+'",description="'+description+'",lastdate="'+lastdate+'" WHERE id="'+id+'"')
    connection.commit()
    return renderAdminShowBloods(request)
@csrf_exempt
def updateEvent(request):
    ...
    connection=mysql.connector.connect(host='localhost',user='root',password='',database='race')
    cursor=connection.cursor()
    print('UPDATE events SET eventTitle="'+title+'",eventDescription="'+desc+'",eventDate="'+date+'" WHERE id="'+id+'"')
    cursor.execute('UPDATE events SET eventTitle="'+title+'",eventDescription="'+desc+'",eventDate="'+date+'" WHERE id="'+id+'"')
    connection.commit()
    return renderAdminShowBloods(request)
...
def insertStudentBlog(request):
    ...
    id=str(id)
    cursor.execute("INSERT INTO `blogs`(`blogtitle`, `blogcontent`, `studentid`) VALUES ('"+title+"','"+content+"','"+id+"')")
    connection.commit()
    return HttpResponse('Success')
